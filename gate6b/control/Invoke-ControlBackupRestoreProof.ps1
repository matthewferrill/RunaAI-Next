...
try {
  ...
  foreach ($spec in $databaseSpecs) {
    ...
    $restoredDump = Join-Path $workRoot "$($spec.Source).restored.dump"
    $sourceSql = Join-Path $workRoot "$($spec.Source).sql"
    $restoredSql = Join-Path $workRoot "$($spec.Source).restored.sql"
    ...
    Invoke-Checked (Join-Path $pgBin 'pg_dump.exe') @('-h','127.0.0.1','-p','9765','-U','postgres',
      '--format=custom','--no-owner','--no-privileges','--file', $restoredDump, $spec.Restore) 'candidate-restored-dump-failed'
    Invoke-Checked (Join-Path $pgBin 'pg_restore.exe') @('--file', $sourceSql, $sourceDump) 'candidate-source-canonicalization-failed'
    Invoke-Checked (Join-Path $pgBin 'pg_restore.exe') @('--file', $restoredSql, $restoredDump) 'candidate-restored-canonicalization-failed'
    $sourceLogicalDigest = Canonical-DumpHash $sourceSql
    $restoredLogicalDigest = Canonical-DumpHash $restoredSql
