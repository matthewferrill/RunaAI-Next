...
try {
  ...
  if (@($credentials | Where-Object { $_.type -eq 'password' }).Count -ne 1 -or
      @($credentials | Where-Object { $_.type -eq 'webauthn-passwordless' }).Count -ne 0) { throw 'owner-rebind-credential-state-mismatch' }
  $env:RUNA_GATE6C_OWNER_SUBJECT=$userId
  $script=Join-Path $Root 'staging\gate6c-localhost-ff15c61\Rebind-ControlOwnerAuthority.mjs'
  $release=Join-Path $Root 'releases\runaai-next-gate6c-localhost-2026-08-22-ff15c61'
  $output=& node.exe $script --release-root $release --config (Join-Path $Root 'config\candidate.json') `
    --expected-release-id 'runaai-next-gate6c-localhost-2026-08-22-ff15c61' `
    --expected-commit 'ff15c618ecbcb5095f362c6055f4a485af3148e7' `
    --expected-artifact-digest 'fff3c379258efe4a2cabf2835c91897c4df528b4ab20b229e967d86a12354668' `
    --prior-release-id 'runaai-next-gate6c-shadow-2026-08-22-ff15c61' `
