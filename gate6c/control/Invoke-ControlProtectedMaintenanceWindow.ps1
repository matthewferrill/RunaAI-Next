...
if($Mode-eq 'Prepare'){
  ...
  [ordered]@{schemaVersion='runa2-gate6cd-prepare/v1';passed=$true;releaseId=$ReleaseId;backupGeneration=$backup.generation;selectedVerifierPassed=$true;freezePreflightPassed=$true;privateValuesIncluded=$false}|ConvertTo-Json -Compress;return
}
