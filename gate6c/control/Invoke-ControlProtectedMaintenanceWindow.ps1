...
function Invoke-Operator([string]$Phase){$output=& $node $operator --phase $Phase --release-root $release --config $config --expected-release-id $ReleaseId --expected-commit $ExpectedCommit --expected-artifact-digest $ExpectedArtifactDigest --legacy-repo $legacy --legacy-commit $legacyCommit --lease-id $LeaseId --restore-proof $restoreProof --preflight-proof $preflightProof 2>&1;$exit=$LASTEXITCODE;$text=($output|ForEach-Object{[string]$_})-join '';if($exit-ne 0){try{$safe=($text|ConvertFrom-Json).errorCode}catch{$safe='gate6d-operator-failed'};throw $safe};if([string]::IsNullOrWhiteSpace($text)){throw 'gate6d-operator-output-invalid'};try{$result=$text|ConvertFrom-Json}catch{throw 'gate6d-operator-output-invalid'};if($null-eq$result-or-not($result.PSObject.Properties.Name-contains 'passed')){throw 'gate6d-operator-output-invalid'};$result}
...
if($Mode-eq 'Prepare'){
  ...
  $httpsResponse=Invoke-WebRequest -UseBasicParsing -Uri "$($authority.Config.publicBaseUrl)/health/ready" -TimeoutSec 10
  if([int]$httpsResponse.StatusCode-ne 200){throw 'gate6d-preflight-private-tls-invalid'}
