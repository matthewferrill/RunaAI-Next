...
test("MXC runs the pinned child or exposes the exact host-preparation blocker and fails closed", { timeout: 30_000 }, async t => {
  ...
  if (receipt.status === "executed") {
    ...
    const ancestorTraversalBlocker = /^sandbox-start-ancestor-(?:[1-6]|path)-denied$/.test(receipt.errorCode)
...
test("public code execution wiring and the compact release runtime preserve the boundary", async () => {
  ...
  assert.match(deployer, /javascriptSource\(content\)/);
  assert.match(deployer, /executionOutput\(receipt\)/);
  assert.match(deployer, /receipt\.status === "executed" \? "Ran in sandbox" : "Not executed"/);
  assert.doesNotMatch(deployer, /Draft — not run/);
