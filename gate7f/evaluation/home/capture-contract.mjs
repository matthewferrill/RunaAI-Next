...
export function requestForCase(item, modelKey, reasoningOff = false) {
  ...
  const request = { model: modelKey, messages: structuredClone(item.messages), temperature: 0,
    max_tokens: item.format === "text" ? 256 : 512, stream: false };
...
export function validateCompletion(response, modelKey, instanceId, runtime) {
  ...
  requireValue(choice.finish_reason === "stop", "gate7f1-provider-output-incomplete");
  requireValue(typeof choice.message?.content === "string" && choice.message.content.trim().length > 0
  ...
  return { rawResponse: choice.message.content, generationTokens: response.usage.completion_tokens,
