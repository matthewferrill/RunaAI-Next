...
requireValue(process.argv[3] === "--authorize-home-burnin-v2", "gate7f1-live-authority-required");
const bundle = JSON.parse(readFileSync(path.join(here, "bundle.json"), "utf8"));
const manifest = bundle.candidates[candidate];
requireValue(manifest && bundle.schemaVersion === "runa2-gate7f1-home-bundle/v2", "gate7f1-bundle-invalid");
...
let inventoryBefore;
const deadline = Date.now() + CAPTURE_POLICY.armTimeoutMs;
try {
  ...
  const runtimeFingerprintSha256 = digest({ runtime: bundle.runtime, loadConfig: loaded.config,
    ...
    corpusSha256: bundle.seal.corpusCanonicalSha256, sealFiles: bundle.seal.files, capturePolicy: CAPTURE_POLICY });
  ...
  requireValue(probe.choices[0].finish_reason === "stop" && probe.choices[0].message.content.trim(), "gate7f1-probe-incomplete");
  for (const item of requests) {
    for (let attempt = 1; attempt <= bundle.runsPerCase; attempt++) {
      ...
      const response = await api("/api/v0/chat/completions", request, CAPTURE_POLICY.requestTimeoutMs);
      ...
      const observation = { schemaVersion: "runa2-gate7f1-observation/v2", candidateId: candidate,
        evaluationSealSha256: bundle.evaluationSealSha256,
  ...
  const result = { schemaVersion: "runa2-gate7f1-home-capture-result/v2", candidate,
