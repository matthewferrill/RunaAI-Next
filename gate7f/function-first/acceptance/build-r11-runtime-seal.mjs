...
const criteriaPath = await realpath(R11_SEAL_AUTHORITIES.criteriaPath);
...
telemetry.classification = R11_SEAL_AUTHORITIES.telemetryClassification;
...
const seal = { ...structuredClone(prior), schemaVersion: "runaai-m1-functional-runtime-seal/v7", sourceCommit,
  ...
  suites, evaluatorId: "codex-r11-sealed-functional-evaluation-20260830",
  qualificationCriteria: { schemaVersion: "runaai-m1-r11-qualification-criteria/v1",
    path: R11_SEAL_AUTHORITIES.criteriaRelativePath, sha256: sha256(criteriaBytes),
    normalizedSha256: sha256(normalize(criteriaBytes)), rubricVersion: R11_SEAL_AUTHORITIES.rubricVersion } };
const manifest = { schemaVersion: "runaai-m1-r11-runtime-seal-input/v1",
  campaignId: R11_SEAL_AUTHORITIES.campaignId, seal,
...
const published = await createR11RuntimeSeal({ manifestPath: inputPath, outputPath: sealPath });
...
console.log(JSON.stringify({ schemaVersion: "runaai-m1-r11-common-build/v1", sourceCommit,
