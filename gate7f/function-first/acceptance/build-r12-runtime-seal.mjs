...
import { createR12RuntimeSeal, R12_SEAL_AUTHORITIES } from "./r12-runtime-seal.mjs";
...
const criteriaPath = await realpath(R12_SEAL_AUTHORITIES.criteriaPath);
...
assert.equal(prior.schemaVersion, "runaai-m1-functional-runtime-seal/v7");
...
telemetry.classification = R12_SEAL_AUTHORITIES.telemetryClassification;
...
const seal = { ...structuredClone(prior), schemaVersion: "runaai-m1-functional-runtime-seal/v8", sourceCommit,
  ...
  suites, evaluatorId: "codex-r12-sealed-functional-evaluation-20260831",
  qualificationCriteria: { schemaVersion: "runaai-m1-r12-qualification-criteria/v1",
    path: R12_SEAL_AUTHORITIES.criteriaRelativePath, sha256: sha256(criteriaBytes),
    normalizedSha256: sha256(normalize(criteriaBytes)), rubricVersion: R12_SEAL_AUTHORITIES.rubricVersion } };
const manifest = { schemaVersion: "runaai-m1-r12-runtime-seal-input/v1",
  campaignId: R12_SEAL_AUTHORITIES.campaignId, seal,
...
const published = await createR12RuntimeSeal({ manifestPath: inputPath, outputPath: sealPath });
...
console.log(JSON.stringify({ schemaVersion: "runaai-m1-r12-common-build/v1", sourceCommit,
