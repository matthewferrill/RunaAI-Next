...
const fail = code => Object.assign(new Error(`m1-r11-seal-${code}`), { code: `m1-r11-seal-${code}` });
const need = (value, code) => { if (!value) throw fail(code); };
const exact = (value, keys) => value && typeof value === "object" && !Array.isArray(value)
  && Object.keys(value).sort().join() === [...keys].sort().join();

export const R11_SEAL_AUTHORITIES = Object.freeze({
  campaignId: "m1-r11-evidence-repair-correction",
  caseBundleSha256: CASE_BUNDLE_SHA256,
  rubricVersion: "2026-08-30.r11-evidence-repair-correction",
  criteriaRelativePath: "gate7f/function-first/M1-S2-R11-CORRECTIVE-CRITERIA-2026-08-30.md",
  criteriaPath: path.resolve(HERE, "../M1-S2-R11-CORRECTIVE-CRITERIA-2026-08-30.md"),
  telemetryClassification: "prospective-r11-hardware-only-not-functional-qualification",
});

export function validateR11Manifest(value) {
  need(exact(value, ["schemaVersion", "campaignId", "seal", "files", "declaration", "privateValuesIncluded"])
    && value.schemaVersion === "runaai-m1-r11-runtime-seal-input/v1"
    && value.campaignId === R11_SEAL_AUTHORITIES.campaignId && value.privateValuesIncluded === false, "manifest");
  const seal = validateRuntimeSeal(value.seal);
  need(seal.schemaVersion === "runaai-m1-functional-runtime-seal/v7"
    && seal.caseBundleSha256 === R11_SEAL_AUTHORITIES.caseBundleSha256
    && seal.sourceCommit !== ACCEPTANCE_POLICY.acceptanceCommit && COMMIT.test(seal.sourceCommit)
    && seal.qualificationCriteria.path === R11_SEAL_AUTHORITIES.criteriaRelativePath
    && seal.qualificationCriteria.rubricVersion === R11_SEAL_AUTHORITIES.rubricVersion
  ...
  need(exact(value.files, ["sourceArchivePath", "packageLockPath", "criteriaPath", "readinessPath",
    ...
    && path.resolve(value.files.criteriaPath).toLowerCase() === R11_SEAL_AUTHORITIES.criteriaPath.toLowerCase(), "files");
  need(exact(value.declaration, ["createdBeforeInference", "sourceArchiveCreatedBeforeInference", "observedR11Attempts",
    ...
    && value.declaration.observedR11Attempts === 0 && value.declaration.importedAttemptCount === 0
...
export async function deriveR11RuntimeSeal({ manifest: input, sourceArchiveBytes, packageLockBytes, criteriaBytes,
  readinessBytes, effectiveReasoningBytes, telemetryBytes }) {
  const manifest = validateR11Manifest(input), seal = manifest.seal;
  ...
  validateRuntimeSealTelemetry(decodeRuntimeSealJson(telemetryBytes), seal,
    R11_SEAL_AUTHORITIES.telemetryClassification, CAMPAIGN_V2_EXTENDED_POLICY);
...
export async function createR11RuntimeSeal({ manifestPath, outputPath }) {
  return publishRuntimeSeal({ manifestPath, outputPath, validateManifest: validateR11Manifest, derive: deriveR11RuntimeSeal,
    publicationSchema: "runaai-m1-r11-runtime-seal-publication/v1" });
}
