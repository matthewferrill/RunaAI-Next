...
import { R11_SEAL_AUTHORITIES, validateR11Manifest } from "./r11-runtime-seal.mjs";
...
const candidates = ACCEPTANCE_POLICY.roster.map((item, index) => ({ candidateId: item.candidateId,
  modelId: `r11-model-${index}`, artifactSha256: String(index + 1).repeat(64), artifactBytes: index + 1,
  requestControls: Object.fromEntries(ACCEPTANCE_POLICY.roles.map(role => [role,
    { reasoningEffort: index === 1 ? null : "none" }])) }));
const seal = () => ({ schemaVersion: "runaai-m1-functional-runtime-seal/v7", sourceCommit,
  ...
  runtime: { nodeSha256: hex, sourceArchiveSha256: hex, packageLockSha256: hex, qdrantSha256: QDRANT_PIN.sha256,
    modelRuntimeSha256: hex, modelRuntimeVersion: "synthetic-r11-runtime" }, candidates,
  ...
  qualificationCriteria: { schemaVersion: "runaai-m1-r11-qualification-criteria/v1",
    path: R11_SEAL_AUTHORITIES.criteriaRelativePath, sha256: hex, normalizedSha256: hex,
    rubricVersion: R11_SEAL_AUTHORITIES.rubricVersion }, evaluatorId: "independent-r11-evaluator",
  maximumBatchMs: CAMPAIGN_V2_EXTENDED_POLICY.maximumBatchMs, productionRoutingChanged: false });
const manifest = () => ({ schemaVersion: "runaai-m1-r11-runtime-seal-input/v1",
  campaignId: R11_SEAL_AUTHORITIES.campaignId, seal: seal(),
  files: { sourceArchivePath: path.resolve("source.tar"), packageLockPath: path.resolve("package-lock.json"),
    criteriaPath: R11_SEAL_AUTHORITIES.criteriaPath, readinessPath: path.resolve("readiness.json"),
    effectiveReasoningPath: path.resolve("reasoning.json"), telemetryPath: path.resolve("telemetry.json") },
  declaration: { createdBeforeInference: true, sourceArchiveCreatedBeforeInference: true, observedR11Attempts: 0,
...
test("R11 seals the full roster, corrected evidence/repair source and unchanged extended campaign window", () => {
  const value = validateR11Manifest(manifest());
  assert.equal(validateRuntimeSeal(value.seal).schemaVersion, "runaai-m1-functional-runtime-seal/v7");
...
test("R11 rejects retrospective evidence, old Review budget, partial roster and inherited seals", () => {
  for (const mutate of [value => value.declaration.observedR11Attempts = 1,
    ...
    const value = manifest(); mutate(value); assert.throws(() => validateR11Manifest(value));
  }
});
