...
import { R12_SEAL_AUTHORITIES, validateR12Manifest } from "./r12-runtime-seal.mjs";
...
const candidates = ACCEPTANCE_POLICY.roster.map((item, index) => ({ candidateId: item.candidateId,
  modelId: `r12-model-${index}`, artifactSha256: String(index + 1).repeat(64), artifactBytes: index + 1,
  requestControls: Object.fromEntries(ACCEPTANCE_POLICY.roles.map(role => [role,
    { reasoningEffort: index === 1 ? null : "none" }])) }));
const seal = () => ({ schemaVersion: "runaai-m1-functional-runtime-seal/v8", sourceCommit,
  ...
  runtime: { nodeSha256: hex, sourceArchiveSha256: hex, packageLockSha256: hex, qdrantSha256: QDRANT_PIN.sha256,
    modelRuntimeSha256: hex, modelRuntimeVersion: "synthetic-r12-runtime" }, candidates,
  ...
  qualificationCriteria: { schemaVersion: "runaai-m1-r12-qualification-criteria/v1",
    path: R12_SEAL_AUTHORITIES.criteriaRelativePath, sha256: hex, normalizedSha256: hex,
    rubricVersion: R12_SEAL_AUTHORITIES.rubricVersion }, evaluatorId: "independent-r12-evaluator",
  maximumBatchMs: CAMPAIGN_V2_EXTENDED_POLICY.maximumBatchMs, productionRoutingChanged: false });
const manifest = () => ({ schemaVersion: "runaai-m1-r12-runtime-seal-input/v1",
  campaignId: R12_SEAL_AUTHORITIES.campaignId, seal: seal(),
  files: { sourceArchivePath: path.resolve("source.tar"), packageLockPath: path.resolve("package-lock.json"),
    criteriaPath: R12_SEAL_AUTHORITIES.criteriaPath, readinessPath: path.resolve("readiness.json"),
    effectiveReasoningPath: path.resolve("reasoning.json"), telemetryPath: path.resolve("telemetry.json") },
  declaration: { createdBeforeInference: true, sourceArchiveCreatedBeforeInference: true, observedR12Attempts: 0,
...
test("R12 seals the combined browser publication source and unchanged full campaign", () => {
  const value = validateR12Manifest(manifest());
  assert.equal(validateRuntimeSeal(value.seal).schemaVersion, "runaai-m1-functional-runtime-seal/v8");
...
test("R12 rejects retrospective evidence, partial rosters and inherited seals", () => {
  for (const mutate of [value => value.declaration.observedR12Attempts = 1,
    value => value.seal.candidates.pop(), value => value.declaration.inheritedRuntimeSealSha256 = hex,
    value => value.seal.schemaVersion = "runaai-m1-functional-runtime-seal/v7"]) {
    const value = manifest(); mutate(value); assert.throws(() => validateR12Manifest(value));
  }
});
