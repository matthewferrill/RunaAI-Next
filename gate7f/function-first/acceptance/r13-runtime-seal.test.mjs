...
import { R13_SEAL_AUTHORITIES, validateR13Manifest } from "./r13-runtime-seal.mjs";
...
const candidates = ACCEPTANCE_POLICY.roster.map((item, index) => ({ candidateId: item.candidateId,
  modelId: `r13-model-${index}`, artifactSha256: String(index + 1).repeat(64), artifactBytes: index + 1,
  requestControls: Object.fromEntries(ACCEPTANCE_POLICY.roles.map(role => [role,
    { reasoningEffort: index === 1 ? null : "none" }])) }));
const seal = () => ({ schemaVersion: "runaai-m1-functional-runtime-seal/v9", sourceCommit,
  ...
  runtime: { nodeSha256: hex, sourceArchiveSha256: hex, packageLockSha256: hex, qdrantSha256: QDRANT_PIN.sha256,
    modelRuntimeSha256: hex, modelRuntimeVersion: "synthetic-r13-runtime" }, candidates,
  ...
  qualificationCriteria: { schemaVersion: "runaai-m1-r13-qualification-criteria/v1",
    path: R13_SEAL_AUTHORITIES.criteriaRelativePath, sha256: hex, normalizedSha256: hex,
    rubricVersion: R13_SEAL_AUTHORITIES.rubricVersion }, evaluatorId: "independent-r13-evaluator",
...
const manifest = () => ({ schemaVersion: "runaai-m1-r13-runtime-seal-input/v1",
  campaignId: R13_SEAL_AUTHORITIES.campaignId, seal: seal(),
  files: { sourceArchivePath: path.resolve("source.tar"), packageLockPath: path.resolve("package-lock.json"),
    criteriaPath: R13_SEAL_AUTHORITIES.criteriaPath, readinessPath: path.resolve("readiness.json"),
...
test("R13 seals the unchanged full campaign to the Agent and Review correction criteria", () => {
  const value = validateR13Manifest(manifest());
  assert.equal(validateRuntimeSeal(value.seal).schemaVersion, "runaai-m1-functional-runtime-seal/v9");
...
test("R13 rejects retrospective evidence, partial rosters and inherited seals", () => {
  for (const mutate of [value => value.declaration.observedR13Attempts = 1,
    value => value.seal.candidates.pop(), value => value.declaration.inheritedRuntimeSealSha256 = hex,
    value => value.seal.schemaVersion = "runaai-m1-functional-runtime-seal/v8"]) {
    const value = manifest(); mutate(value); assert.throws(() => validateR13Manifest(value));
  }
});
