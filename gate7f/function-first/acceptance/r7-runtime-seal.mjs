...
export const normalizedRuntimeSealBytes = bytes => Buffer.from(new TextDecoder("utf8", { fatal: true }).decode(bytes).replaceAll("\r\n", "\n"));
export const decodeRuntimeSealJson = bytes => { try { return JSON.parse(new TextDecoder("utf8", { fatal: true }).decode(bytes)); } catch { throw fail("json"); } };
...
export function expectedRuntimeSealSuites() {
  const result = {};
...
export function validateRuntimeSealReadiness(value) {
...
export async function boundedRuntimeSealFile(filename, maximum) {
...
export async function deriveR7RuntimeSeal({ manifest: input, sourceArchiveBytes, packageLockBytes, criteriaBytes,
  ...
  validateRuntimeSealReadiness(decodeRuntimeSealJson(readinessBytes)); validateRuntimeSealReadiness(decodeRuntimeSealJson(effectiveReasoningBytes));
...
export async function publishRuntimeSeal({ manifestPath, outputPath, validateManifest, derive, publicationSchema }) {
  const manifestBytes = await boundedRuntimeSealFile(manifestPath, 1024 * 1024), manifest = validateManifest(decodeRuntimeSealJson(manifestBytes));
  const [sourceArchiveBytes, packageLockBytes, criteriaBytes, readinessBytes, effectiveReasoningBytes, telemetryBytes] = await Promise.all([
    boundedRuntimeSealFile(manifest.files.sourceArchivePath, 512 * 1024 * 1024), boundedRuntimeSealFile(manifest.files.packageLockPath, 16 * 1024 * 1024),
    boundedRuntimeSealFile(manifest.files.criteriaPath, 1024 * 1024), boundedRuntimeSealFile(manifest.files.readinessPath, 16 * 1024 * 1024),
    boundedRuntimeSealFile(manifest.files.effectiveReasoningPath, 16 * 1024 * 1024), boundedRuntimeSealFile(manifest.files.telemetryPath, 16 * 1024 * 1024),
  ]);
  const result = await derive({ manifest, sourceArchiveBytes, packageLockBytes, criteriaBytes,
    readinessBytes, effectiveReasoningBytes, telemetryBytes });
  ...
  return Object.freeze({ schemaVersion: publicationSchema, outputPath: target,
...
export async function createR7RuntimeSeal({ manifestPath, outputPath }) {
  return publishRuntimeSeal({ manifestPath, outputPath, validateManifest: validateR7Manifest,
    derive: deriveR7RuntimeSeal, publicationSchema: "runaai-m1-r7-runtime-seal-publication/v1" });
}
