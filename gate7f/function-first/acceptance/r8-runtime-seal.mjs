...
import {
  decodeRuntimeSealJson,
  expectedRuntimeSealSuites,
  normalizedRuntimeSealBytes,
  publishRuntimeSeal,
  validateRuntimeSealReadiness,
  validateRuntimeSealTelemetry,
} from "./r7-runtime-seal.mjs";

const HERE = path.dirname(fileURLToPath(import.meta.url));
const COMMIT = /^[a-f0-9]{40}$/u;
const fail = code => Object.assign(new Error(`m1-r8-seal-${code}`), { code: `m1-r8-seal-${code}` });
const need = (value, code) => { if (!value) throw fail(code); };
const exact = (value, keys) => value && typeof value === "object" && !Array.isArray(value)
  && Object.keys(value).sort().join() === [...keys].sort().join();

export const R8_SEAL_AUTHORITIES = Object.freeze({
  campaignId: "m1-r8-two-phase-browser-witness",
  caseBundleSha256: CASE_BUNDLE_SHA256,
  rubricVersion: "2026-08-30.r8-two-phase-browser-witness",
  criteriaRelativePath: "gate7f/function-first/M1-S2-R8-TWO-PHASE-BROWSER-WITNESS-CRITERIA-2026-08-30.md",
  criteriaPath: path.resolve(HERE, "../M1-S2-R8-TWO-PHASE-BROWSER-WITNESS-CRITERIA-2026-08-30.md"),
  telemetryClassification: "prospective-r8-hardware-only-not-functional-qualification",
});

export function validateR8Manifest(value) {
  need(exact(value, ["schemaVersion", "campaignId", "seal", "files", "declaration", "privateValuesIncluded"])
    && value.schemaVersion === "runaai-m1-r8-runtime-seal-input/v1"
    && value.campaignId === R8_SEAL_AUTHORITIES.campaignId && value.privateValuesIncluded === false, "manifest");
  const seal = validateRuntimeSeal(value.seal);
  need(seal.schemaVersion === "runaai-m1-functional-runtime-seal/v4"
    && seal.caseBundleSha256 === R8_SEAL_AUTHORITIES.caseBundleSha256
    && seal.sourceCommit !== ACCEPTANCE_POLICY.acceptanceCommit && COMMIT.test(seal.sourceCommit)
    && seal.qualificationCriteria.path === R8_SEAL_AUTHORITIES.criteriaRelativePath
    && seal.qualificationCriteria.rubricVersion === R8_SEAL_AUTHORITIES.rubricVersion
  ...
  need(exact(value.files, ["sourceArchivePath", "packageLockPath", "criteriaPath", "readinessPath", "effectiveReasoningPath", "telemetryPath"])
    ...
    && path.resolve(value.files.criteriaPath).toLowerCase() === R8_SEAL_AUTHORITIES.criteriaPath.toLowerCase(), "files");
  need(exact(value.declaration, ["createdBeforeInference", "sourceArchiveCreatedBeforeInference", "observedR8Attempts",
    ...
    && value.declaration.observedR8Attempts === 0 && value.declaration.importedAttemptCount === 0
...
export async function deriveR8RuntimeSeal({ manifest: input, sourceArchiveBytes, packageLockBytes, criteriaBytes,
  readinessBytes, effectiveReasoningBytes, telemetryBytes }) {
  const manifest = validateR8Manifest(input), seal = manifest.seal;
  ...
  validateRuntimeSealTelemetry(decodeRuntimeSealJson(telemetryBytes), seal, R8_SEAL_AUTHORITIES.telemetryClassification);
...
export async function createR8RuntimeSeal({ manifestPath, outputPath }) {
  return publishRuntimeSeal({ manifestPath, outputPath, validateManifest: validateR8Manifest,
    derive: deriveR8RuntimeSeal, publicationSchema: "runaai-m1-r8-runtime-seal-publication/v1" });
}
