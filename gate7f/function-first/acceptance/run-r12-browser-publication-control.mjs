...
const safeCode = (error, label = "r12") => /^[a-z0-9-]{1,100}$/u.test(error?.code ?? "")
  ? error.code : `m1-${label}-browser-control-failed`;

export function parseBrowserPublicationControlArguments(argv, { label }) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!raw?.startsWith("--") || !allowed.has(key) || !value || value.startsWith("--") || seen.has(key)) {
      throw fail(`m1-${label}-browser-control-argument-invalid`);
    }
...
export function parseR12BrowserControlArguments(argv) {
  return parseBrowserPublicationControlArguments(argv, { label: "r12" });
}

export async function runBrowserPublicationControl(args, { announce = value => process.stdout.write(`${JSON.stringify(value)}\n`),
  label, sealVersion, reportSchema, identitySeedSuffix } = {}) {
  const denied = suffix => fail(`m1-${label}-browser-control-${suffix}`);
  ...
  if (!sealPath.startsWith(root + path.sep)) throw denied("seal-path-denied");
  ...
  if (!sealInfo.isFile() || sealInfo.isSymbolicLink() || sealInfo.size > 262144) throw denied("seal-invalid");
  ...
  if (seal.schemaVersion !== sealVersion || identity.sourceCommit !== args["source-commit"]
  ...
  const report = { schemaVersion: reportSchema, sourceCommit: identity.sourceCommit,
  ...
  try {
    ...
    const journey = new FunctionalHttpJourney({ host: testbed.host, item, ledger,
      identitySeed: `${runtimeSealSha256}/${identitySeedSuffix}`, checkpoint });
    ...
    if (prepared?.preparationOnly !== true) throw denied("preparation-unproven");
    ...
    if (!report.actualBrowserExercised || !report.witnessOnTime || !report.acknowledgementOnTime
      ...
      throw denied("criteria-unmet");
    }
    report.passed = true;
  } catch (error) {
    report.passed = false; report.errorCode = safeCode(error, label);
  } finally {
    try { if (pending) await pending; } catch {}
    try { await testbed?.close(); await resources?.close(); } catch (error) { report.cleanupError = safeCode(error, label); }
    report.healthDiagnostics = healthCaptureDiagnostics(testbed?.transports);
    report.finishedAt = stamp();
    const filename = `${label}-browser-publication-control-${Date.now()}.json`;
...
export function runR12BrowserPublicationControl(args, options = {}) {
  return runBrowserPublicationControl(args, { ...options, label: "r12",
    sealVersion: "runaai-m1-functional-runtime-seal/v8",
    reportSchema: "runaai-m1-r12-browser-publication-control/v1",
    identitySeedSuffix: "r12-model-free-browser-publication" });
}
