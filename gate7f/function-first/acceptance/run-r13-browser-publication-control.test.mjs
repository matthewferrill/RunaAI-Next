...
import { parseR13BrowserControlArguments } from "./run-r13-browser-publication-control.mjs";
...
test("R13 model-free actual-browser control has one exact invocation surface", () => {
  assert.deepEqual(parseR13BrowserControlArguments(valid), { "owned-root": valid[1], "source-commit": commit,
  ...
  for (const changed of [valid.slice(0, -2), [...valid, "--case-id", "agent-05-cancel-drain"],
    ...
    assert.throws(() => parseR13BrowserControlArguments(changed), /m1-r13-browser-control-argument-invalid/u);
  }
});

test("R13 control binds v9 and reuses only the source-reviewed model-free executor", async () => {
  const source = await readFile(new URL("./run-r13-browser-publication-control.mjs", import.meta.url), "utf8");
  const executor = await readFile(new URL("./run-r12-browser-publication-control.mjs", import.meta.url), "utf8");
  assert.match(source, /runaai-m1-functional-runtime-seal\/v9/u);
