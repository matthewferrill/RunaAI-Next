...
export const REVIEW_VERIFICATION_SCHEMA=freeze({$schema:'http://json-schema.org/draft-07/schema#',type:'object',
  additionalProperties:false,properties:{verdict:{type:'string',enum:['accept','revise']},
    reason:{type:'string',minLength:1},finalAnswer:{type:'string',minLength:1},
    citations:{type:'array',minItems:1,items:CITATION_SCHEMA}},
  required:['verdict','reason','finalAnswer','citations']});
export const REVIEW_VERIFICATION_STRUCTURED_OUTPUT=freeze({schema:REVIEW_VERIFICATION_SCHEMA,
  errorStrategy:'error',jsonPromptInjection:false});
export const REVIEW_VERIFICATION_RESPONSE_FORMAT=freeze({type:'json_schema',
  json_schema:{schema:REVIEW_VERIFICATION_SCHEMA,strict:true,name:'response'}});
...
export function isReviewVerificationResponseFormat(value){return canonical(value)===canonical(REVIEW_VERIFICATION_RESPONSE_FORMAT);}
export function isQualifiedEvidenceResponseFormat(value){return isEvidenceResponseFormat(value)
  ||isEvidenceVerificationResponseFormat(value)||isReviewVerificationResponseFormat(value);}
