...
$root=$PSScriptRoot
. ($root+'\Runtime-Windows.ps1')
$script:RuntimeRoot=[IO.Path]::GetDirectoryName($root)
...
$seal=Get-Content -LiteralPath ($root+'\seal.json') -Raw|ConvertFrom-Json;$config=Get-Content -LiteralPath ($root+'\config.json') -Raw|ConvertFrom-Json;$output=[string]$config.outputRoot
