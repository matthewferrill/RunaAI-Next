import {execFileSync} from 'node:child_process';import {writeFileSync,existsSync} from 'node:fs';import path from 'node:path';
import {tlsTransportRequest} from './tls-enrollment-operator.mjs';import {demand,sha} from './tls-primitives.mjs';
const output=path.resolve(process.argv[2]??'');demand(process.argv.length===3&&path.isAbsolute(output)&&!existsSync(output),'processing-proof-preflight-args');
const command=`$ErrorActionPreference='Stop';$ProgressPreference='SilentlyContinue';Set-StrictMode -Version Latest;
if($env:COMPUTERNAME-cne'RUNA-HOME'){throw 'processing-proof-preflight-host'};
$enginePath='C:\\Users\\Matthew\\AppData\\Local\\Programs\\LM Studio\\LM Studio.exe';$cli='C:\\Users\\Matthew\\.lmstudio\\bin\\lms.exe';$descriptor='C:\\Users\\Matthew\\.lmstudio\\.internal\\http-server-port';$node='C:\\Program Files\\nodejs\\node.exe';
$engines=@(Get-Process 'LM Studio' -ErrorAction Stop|Where-Object{$_.Path-ceq$enginePath});if($engines.Count-ne1){throw 'processing-proof-preflight-engine'};$engine=$engines[0];
...
demand(value?.schemaVersion==='runaai-native-processing-proof-preflight/v1'&&value.host==='RUNA-HOME'&&value.residentCount===0
  ...
  &&['engineSha256','cliSha256','descriptorSha256'].every(key=>/^[a-f0-9]{64}$/.test(value[key]))
  &&/^[a-f0-9]{64}$/.test(value.node.sha256)&&value.engine?.executable==='C:\\Users\\Matthew\\AppData\\Local\\Programs\\LM Studio\\LM Studio.exe'
