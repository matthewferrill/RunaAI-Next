param([ValidateSet('Upload','Inventory','Run','PowerRun','Export','Status','Result','Recovery','Probe','ProbeStatus','ProbeCleanup','ProbeExport','AuxiliaryExport','Dispatch','TaskStatus','TaskCleanup')][string]$Mode,
...
if($Mode-eq'Dispatch') {
...
} elseif($Mode-eq'AuxiliaryExport') {
 $remote += '[Console]::Out.Write([IO.File]::ReadAllText(''C:\Users\codex-audit\AppData\Local\RunaM1Readiness\20260828-readiness-r4\auxiliary-pin.json''))'
 ...
 $remote += 'Get-Content -LiteralPath '''+$remoteRoot+'\result-'+$Candidate+'.json'' -Raw'
...
if($Mode -in @('Export','ProbeExport','AuxiliaryExport')) {
...
if($Mode -notin @('Export','ProbeExport','AuxiliaryExport')) { while(-not $process.StandardOutput.EndOfStream){ [Console]::Out.WriteLine($process.StandardOutput.ReadLine()) } }
$process.WaitForExit()
if($Mode -in @('Export','ProbeExport','AuxiliaryExport')){try{$null=$copy.GetAwaiter().GetResult()}finally{$out.Dispose()}}
