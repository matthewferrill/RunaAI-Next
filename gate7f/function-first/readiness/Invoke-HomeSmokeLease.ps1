...
if($Mode-eq'Dispatch'){
 ...
 $remote+='$registry=Invoke-RestMethod -Uri ''http://127.0.0.1:1234/api/v1/models'' -TimeoutSec 10;$models=@($registry.models|ForEach-Object{@{key=$_.key;loadedInstances=@($_.loaded_instances)}});$gpus=@(& nvidia-smi.exe --query-gpu=index,uuid,power.limit,temperature.gpu,memory.total,memory.used --format=csv,noheader,nounits);$tasks=@(Get-ScheduledTask|Where-Object{$_.TaskName-like''Runa-M1-20260828-smoke-*''}|Select-Object TaskName,State);$ports=@(Get-NetTCPConnection -State Listen|Where-Object{$_.LocalPort-in@(1234,8412)}|Select-Object LocalAddress,LocalPort);[Console]::Out.Write((@{schemaVersion=''runa-m1-smoke-final-observation/v1'';time=[DateTime]::UtcNow.ToString(''o'');host=$env:COMPUTERNAME;models=$models;gpus=$gpus;ownedTaskRegistrations=$tasks;listeners=$ports;readOnly=$true;protectedDataIncluded=$false}|ConvertTo-Json -Depth 20))'
