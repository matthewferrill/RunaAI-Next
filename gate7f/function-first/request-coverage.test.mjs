...
import {EVIDENCE_RESPONSE_FORMAT,EVIDENCE_VERIFICATION_RESPONSE_FORMAT,REVIEW_VERIFICATION_RESPONSE_FORMAT} from './evidence-output.mjs';
...
for(const candidate of MANIFEST.candidates)for(const mode of modes)test(`same continuing-constraint instructions reach actual ${mode.name} wire for ${candidate.id}`,async()=>{
  ...
  try{
    ...
    if(['research','review'].includes(mode.role)){
      ...
      assert.deepEqual(verification.response_format,mode.role==='review'
        ?REVIEW_VERIFICATION_RESPONSE_FORMAT:EVIDENCE_VERIFICATION_RESPONSE_FORMAT);
    }
    ...
    assert.equal(denied.completion.reason,'effect-policy-suspension-denied');assert.equal(wire.length,expectedCalls);assert.deepEqual(denied.effects,[]);
