...
export class M1RoleOrchestrator {
  ...
  async start(context, input, options = {}) {
    const { workflow = "code", ...request } = startSchema.parse(input);
    return this[workflow].start(context, request, options);
  }
  async resume(context, input, options = {}) {
    ...
    return this[role].resume(context, input, options);
  }
