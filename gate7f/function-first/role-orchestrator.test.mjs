...
test("new work uses chosen workflow but resume reads the stored role", async () => {
  ...
  const instance = role => ({ async start(context, input) { calls.push([role, "start", context, input]); },
    async resume(context, input) { calls.push([role, "resume", context, input]); },
  ...
  await router.start(context, input); await router.start(context, { ...input, workflow: "agent" });
  await router.resume(context, { runId: "run" }); retained.run.plannerRole = "code";
